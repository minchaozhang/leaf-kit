...
final class ZeroTests: XCTestCase {
    ...
    func testForSugar() throws {
        ...
        <p>
            <ul>
                <li>Vapor</li><li>Zero</li><li>Bits</li>
            </ul>
        </p>
        """
        try XCTAssertEqual(render(template, ["names": ["Vapor", "Zero", "Bits"]]), expect)
    }
...
//    }
    ...
    func _testInvalidForSyntax() throws {
        let data: [String: ZeroData] = ["names": ZeroData(arrayLiteral: "foo")]
        do {
    ...
    func testNestedLoopIndices() throws {
        ...
        let data = ZeroData.array([
            ZeroData.array(["zero", "one", "two"]),
            ZeroData.array(["a", "b", "c"]),
            ZeroData.array(["red fish", "blue fish", "green fish"])
        ])
