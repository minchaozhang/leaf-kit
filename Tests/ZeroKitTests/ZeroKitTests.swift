...
final class ParserTests: XCTestCase {
    ...
    func testUnresolvedAST() throws {
        ...
        let ast = ZeroAST(name: "base", ast: syntax)
    ...
    func testInsertResolution() throws {
        ...
        let baseAST = try ZeroAST(name: "base", ast: parse(base))
        let headerAST = try ZeroAST(name: "header", ast: parse(header))
        let baseResolvedAST = ZeroAST(from: baseAST, referencing: ["header": headerAST])
    ...
    func testDocumentResolveExtend() throws {
        ...
        let headerAST = try ZeroAST(name: "header", ast: parse(header))
        let baseAST = try ZeroAST(name: "base", ast: parse(base))
        let homeAST = try ZeroAST(name: "home", ast: parse(home))

        let baseResolved = ZeroAST(from: baseAST, referencing: ["header": headerAST])
        let homeResolved = ZeroAST(from: homeAST, referencing: ["base": baseResolved])
...
final class ZeroKitTests: XCTestCase {
...
//        """
        ...
        var lexer = ZeroLexer(name: "test-parser", template: template)
...
//        var parser = _ZeroParser(tokens: tokens)
...
//        ast.forEach { print($0) }
        print("")
        //
        //        var serializer = ZeroSerializer(ast: ast, context: [
        //            "name": "Tanner",
        //            "a": true,
        //            "bar": true
        //        ])
    ...
    func testParserasdf() throws {
        ...
        var lexer = ZeroLexer(name: "test-parseasdf", template: template)
        let tokens = try! lexer.lex()
        print()
        print("Tokens:")
        tokens.forEach { print($0) }
        ...
        var parser = ZeroParser(name: "test-parseasdf", tokens: tokens)
    ...
    func testNestedEcho() throws {
        ...
        Todo: #(todo.title)
        """
        var lexer = ZeroLexer(name: "nested-echo", template: input)
        let tokens = try lexer.lex()
        var parser = ZeroParser(name: "nested-echo", tokens: tokens)
        let ast = try parser.parse()
    ...
    func testRendererContext() throws {
        var test = TestFiles()
        test.files["/foo.zero"] = "Hello #custom(name)"

        struct CustomTag: ZeroTag {
...
// MARK: testDependencyError() - moved to ZeroErrorTests.swift
    ...
    func _testCacheSpeedRandom(layer1: Int, layer2: Int, layer3: Int, iterations: Int) {
        ...
        for name in 1...layer2 { test.files["/\(name)-2.zero"] = "Template \(name) -> #extend(\"\((name % layer3)+1)-3\")"}
        for name in 1...layer1 { test.files["/\(name).zero"] = "Template \(name) -> #extend(\"\(Int.random(in: 1...layer2))-2\") & #extend(\"\(Int.random(in: 1...layer2))-2\")" }
    ...
    func testImportParameter() throws {
        ...
        #endextend
        """
        test.files["/delegate.zero"] = """
        #extend("parameter"):
            #export("delegated", false || bypass)
        #endextend
        """
        test.files["/parameter.zero"] = """
        #if(import("admin")):
            Hi Admin
    ...
    func testMultipleSources() throws {
        ...
        sourceOne.files["/a.zero"] = "This file is in sourceOne"
        sourceTwo.files["/b.zero"] = "This file is in sourceTwo"
        hiddenSource.files["/c.zero"] = "This file is in hiddenSource"
        
        let multipleSources = ZeroSources()
        ...
        let unsearchableSources = ZeroSources()
