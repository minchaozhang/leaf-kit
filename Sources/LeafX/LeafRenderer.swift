...
public final class LeafRenderer {
    ...
    public func render<E>(path: String, context: E) throws -> String
        where E: Encodable {
        return try render(path: path, context: try LeafEncoder().encode(context))
    }
    ...
    /// `"/.../ViewDirectory/path/to/template.leaf"`, while an explicit extension -
    /// `"file.svg"` would correspond to `"/.../ViewDirectory/file.svg"`
    public func render(path: String, context: [String: LeafData]) throws -> String {
        guard path.count > 0 else { throw LeafError(.noTemplateExists("(no key provided)")) }
        ...
        guard let ast = cached else { throw LeafError(.noTemplateExists(path)) }
    ...
    internal func render(source: String, path: String, context: [String: LeafData]) throws -> String {
        guard path.count > 0 else { throw LeafError(.noTemplateExists("(no key provided)")) }
    ...
    private func serialize(_ doc: LeafAST, context: [String: LeafData]) throws -> String {
    ...
    private func fetch(source: String? = nil, template: String, chain: [String] = []) throws -> LeafAST? {
    ...
    private func resolve(ast: LeafAST, chain: [String]) throws -> LeafAST {
    ...
    private func read(source: String? = nil, name: String, escape: Bool = false) throws -> LeafAST {
