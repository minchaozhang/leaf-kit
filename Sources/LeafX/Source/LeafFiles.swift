...
public struct LeafFiles: LeafSource {
    // MARK: - Public
    
    /// Various options for configuring an instance of `LeafFiles`
    ///
    ...
    ///
    /// A new `LeafFiles` defaults to [.toSandbox, .toVisibleFiles, .requireExtensions]
    ...
    /// Initialize `LeafFiles` with a NIO file IO object, limit options, and sandbox/view dirs
    /// - Parameters:
    ///   - limits: Options for constraining which files may be read - see `LeafFiles.Limit`
