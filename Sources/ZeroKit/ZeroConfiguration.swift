...
public struct ZeroConfiguration {
    ...
    /// Initialize Zero with a specific tagIndicator
    /// - Parameter rootDirectory: Default directory where templates will be found
    ...
    internal var _rootDirectory: String {
        willSet { assert(!accessed, "Changing property after ZeroConfiguration has been read has no effect") }
    }
    ...
    private static var running: Bool {
        assert(!Self.started, "ZeroKit can only be configured prior to instantiating any ZeroRenderer")
        return Self.started
    }
