...
internal struct ZeroParser {
    ...
    init(name: String, tokens: [ZeroToken]) {
        self.name = name
        self.tokens = tokens
        self.offset = 0
    }
    ...
    private var tokens: [ZeroToken]
    private var offset: Int
    
    private var finished: [Syntax] = []
    private var awaitingBody: [OpenContext] = []

    private mutating func handle(next: ZeroToken) throws {
        switch next {
        case .tagIndicator:
    ...
    private mutating func replace(at offset: Int = 0, with new: ZeroToken) {
        self.tokens[self.offset + offset] = new
    }

    private mutating func read() -> ZeroToken? {
    ...
    private mutating func readWhile(_ check: (ZeroToken) -> Bool) -> [ZeroToken]? {
        guard self.offset < self.tokens.count else { return nil }
        var matched = [ZeroToken]()
