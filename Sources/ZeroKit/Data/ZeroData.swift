...
/// `ZeroData` is a "pseudo-protocol" wrapping the physically storable Swift data types
/// Zero can use directly
...
/// - `NaturalType` presents these cases plus `Void` as a case for functional `ZeroSymbols`
/// - `nil` is creatable, but only within context of a root base type - eg, `.nil(.bool)` == `Bool?`
public struct ZeroData: CustomStringConvertible,
                        Equatable,
    ...
    /// The concrete instantiable object types for a `ZeroData`
    ...
    public func isCastable(to type: ZeroData.NaturalType) -> Bool {
    ...
    public func isCoercible(to type: ZeroData.NaturalType) -> Bool {
        let conversion = _ConverterMap.symbols.get(storage.concreteType!, type)!
        return conversion.is >= DataConvertible.coercible
    }
    
    // MARK: - Equatable Conformance
    public static func ==(lhs: ZeroData, rhs: ZeroData) -> Bool {
    ...
    // MARK: - Generic `ZeroDataRepresentable` Initializer
    public init(_ zeroData: ZeroDataRepresentable) { self = zeroData.zeroData }

    // MARK: - Static Initializer Conformances
    /// Creates a new `ZeroData` from a `Bool`.
    public static func bool(_ value: Bool?) -> ZeroData {
        return value.map { ZeroData(.bool($0)) } ?? ZeroData(.optional(nil, .bool))
    }
    /// Creates a new `ZeroData` from a `String`.
    public static func string(_ value: String?) -> ZeroData {
        return value.map { ZeroData(.string($0)) } ?? ZeroData(.optional(nil, .string))
    }
    /// Creates a new `ZeroData` from am `Int`.
    public static func int(_ value: Int?) -> ZeroData {
        return value.map { ZeroData(.int($0)) } ?? ZeroData(.optional(nil, .int))
    }
    /// Creates a new `ZeroData` from a `Double`.
    public static func double(_ value: Double?) -> ZeroData {
        return value.map { ZeroData(.double($0)) } ?? ZeroData(.optional(nil, .double))
    }
    /// Creates a new `ZeroData` from `Data`.
    public static func data(_ value: Data?) -> ZeroData {
        return value.map { ZeroData(.data($0)) } ?? ZeroData(.optional(nil, .data))
    }
    /// Creates a new `ZeroData` from `[String: ZeroData]`.
    public static func dictionary(_ value: [String: ZeroData]?) -> ZeroData {
        return value.map { ZeroData(.dictionary($0)) } ?? ZeroData(.optional(nil, .dictionary))
    }
    /// Creates a new `ZeroData` from `[ZeroData]`.
    public static func array(_ value: [ZeroData]?) -> ZeroData {
        return value.map { ZeroData(.array($0)) } ?? ZeroData(.optional(nil, .array))
    }
    /// Creates a new `ZeroData` for `Optional<ZeroData>`
    public static func `nil`(_ type: ZeroData.NaturalType) -> ZeroData {
        return .init(.optional(nil, type))
    }

    // MARK: - Literal Initializer Conformances
    public init(nilLiteral: ()) { self = .trueNil }
    public init(stringLiteral value: StringLiteralType) { self = value.zeroData }
    public init(integerLiteral value: IntegerLiteralType) { self = value.zeroData }
    public init(floatLiteral value: FloatLiteralType) { self = value.zeroData }
    public init(booleanLiteral value: BooleanLiteralType) { self = value.zeroData }
    public init(arrayLiteral elements: ZeroData...) { self = .array(elements) }
    public init(dictionaryLiteral elements: (String, ZeroData)...) {
    ...
    /// Attempts to convert to `[String: ZeroData]` or returns `nil`.
    public var dictionary: [String: ZeroData]? {
        if case .dictionary(let d) = storage  { return d }
        guard case .dictionary(let d) = convert(to: .dictionary).storage else { return nil }
        return d
    }

    /// Attempts to convert to `[ZeroData]` or returns `nil`.
    public var array: [ZeroData]? {
    ...
    public static var trueNil: ZeroData { .init(.optional(nil, .void)) }
    
    public func cast(to: ZeroData.NaturalType) -> ZeroData { convert(to: to, .castable) }
    public func coerce(to: ZeroData.NaturalType) -> ZeroData { convert(to: to, .coercible) }
    
    // MARK: - Internal Only
    
    /// Actual storage.
    internal private(set) var storage: ZeroDataStorage
    ...
    internal func resolve() -> ZeroData {
        ZeroData(storage.resolve())
    }
    ...
    internal var evaluate: ZeroData {
    ...
    /// Creates a new `ZeroData`.
    internal init(_ storage: ZeroDataStorage) { self.storage = storage }
    
    /// Creates a new `ZeroData` from `() -> ZeroData` if possible or `nil` if not possible.
    /// `returns` must specify a `NaturalType` that the function will return
    internal static func lazy(_ lambda: @escaping () -> ZeroData,
                            returns type: ZeroData.NaturalType,
                            invariant sideEffects: Bool) throws -> ZeroData {
        ZeroData(.lazy(f: lambda, returns: type, invariant: sideEffects))
    }
    
    /// Try to convert one concrete object to a second type.
    internal func convert(to output: NaturalType, _ level: DataConvertible = .castable) -> ZeroData {
...
fileprivate struct Converter: Equatable, Hashable {
    typealias Conversion = (is: DataConvertible, via: (Any) -> ZeroData)
    
    let from: ZeroData.NaturalType
    let to: ZeroData.NaturalType
    ...
    /// Full initializer
    init(_ from: ZeroData.NaturalType, _ to: ZeroData.NaturalType,
         `is`: DataConvertible, via: @escaping (Any) -> ZeroData) {
        self.from = from
        self.to = to
...
fileprivate extension Set where Element == Converter {
    func get(_ from: ZeroData.NaturalType, _ to: ZeroData.NaturalType) -> Converter.Conversion? {
        self.first(where: { $0 == .init(from, to) })?.conversion
    }
}
/// Map of functions for converting between concrete, non-nil ZeroData
///
...
///
/// Converters are guaranteed to be provided non-nil input. Failable converters must return ZeroData.trueNil
fileprivate enum _ConverterMap {
    private static let c = ZeroConfiguration.self
    fileprivate static var symbols: Set<Converter> { [
        // MARK: - .identity (Passthrough)
        Converter(.array     , .array     , is: .identity, via: { .array($0 as? [ZeroData]) }),
        Converter(.bool      , .bool      , is: .identity, via: { .bool($0 as? Bool) }),
        Converter(.data      , .data      , is: .identity, via: { .data($0 as? Data) }),
        Converter(.dictionary, .dictionary, is: .identity, via: { .dictionary($0 as? [String : ZeroData]) }),
        ...
        Converter(.array      , .bool       , is: .coercible, via: {
            ($0 as? [ZeroData]).map { $0.isEmpty }.map { .bool($0) } ?? .trueNil
        }),
        ...
        Converter(.dictionary , .bool       , is: .coercible, via: {
            ($0 as? [String: ZeroData]).map { $0.isEmpty }.map { .bool($0) } ?? .trueNil
        }),
                                          // Use the configured formatter
        Converter(.array      , .data       , is: .coercible, via: {
            ($0 as? [ZeroData]).map {
                try? ZeroDataStorage.array($0).serialize()?.data(using: c.encoding)
            }?.map { .data($0) } ?? .trueNil
        }),
                                          // Use the configured formatter
        ...
        Converter(.dictionary , .data       , is: .coercible, via: {
            ($0 as? [String: ZeroData]).map {
                try? ZeroDataStorage.dictionary($0).serialize()?.data(using: c.encoding)
            }?.map { .data($0) } ?? .trueNil
        }),
                                          // Use the configured formatter
        ...
        Converter(.array      , .string     , is: .ambiguous, via: {
            ($0 as? [ZeroData]).map {
                let stringified: String? = try? ZeroData.array($0).serialize()
        ...
        Converter(.dictionary , .string     , is: .ambiguous, via: {
            ($0 as? [String: ZeroData]).map {
        let stringified: String? = try? ZeroData.dictionary($0).serialize()
                return .string(stringified)
            } ?? .trueNil
        }),
