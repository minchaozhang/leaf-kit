internal final class ZeroEncoder {
    func encode<E>(_ encodable: E) throws -> [String: ZeroData] where E: Encodable {
...
private final class SingleValueContainer: SingleValueEncodingContainer, _Container {
    ...
    func encode<T>(_ value: T) throws where T: Encodable {
        if let zeroRepresentable = value as? ZeroDataRepresentable {
            self.data = .data(zeroRepresentable.zeroData)
        } else {
    ...
    where Key: CodingKey
{
    ...
    {
        if let zeroRepresentable = value as? ZeroDataRepresentable {
            self.dictionary[key.stringValue] = .data(zeroRepresentable.zeroData)
        } else {
...
private final class UnkeyedContainer: UnkeyedEncodingContainer, _Container {
    ...
    func encode<T>(_ value: T) throws where T: Encodable {
        defer { self.count += 1 }
        if let zeroRepresentable = value as? ZeroDataRepresentable {
            self.array.append(.data(zeroRepresentable.zeroData))
        } else {
