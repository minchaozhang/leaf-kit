...
// MARK: `ZeroToken` Summary

/// `ZeroToken` represents the first stage of parsing Zero templates - a raw file/bytestream `String`
/// will be read by `ZeroLexer` and interpreted into `[ZeroToken]` representing a stream of tokens.
///
/// # STABLE TOKEN DEFINITIONS
/// - `.raw`: A variable-length string of data that will eventually be output directly without processing
/// - `.tagIndicator`: The signal at top-level that a Zero syntax object will follow. Default is `#` and
...
///
/// # TODO
/// - ZeroTokens would ideally also store the range of their location in the original source template

internal enum ZeroToken: CustomStringConvertible, Equatable  {
    /// Holds a variable-length string of data that will be passed through with no processing
    case raw(String)
    
    /// `#` (or as configured) - Top-level signal that indicates a Zero tag/syntax object will follow.
    case tagIndicator
