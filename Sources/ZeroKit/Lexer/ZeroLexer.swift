...
// MARK: `ZeroLexer` Summary

/// `ZeroLexer` is an opaque structure that wraps the lexing logic of Zero-Kit.
///
/// Initialized with a `ZeroRawTemplate` (raw string-providing representation of a file or other source),
/// used by evaluating with `ZeroLexer.lex()` and either erroring or returning `[ZeroToken]`
internal struct ZeroLexer {
    // MARK: - Internal Only
    
    /// Convenience to initialize `ZeroLexer` with a `String`
    init(name: String, template string: String) {
        self.name = name
        self.src = ZeroRawTemplate(name: name, src: string)
        self.state = .raw
    }
    
    /// Init with `ZeroRawTemplate`
    init(name: String, template: ZeroRawTemplate) {
        self.name = name
        self.src = template
        self.state = .raw
    }
    
    /// Lex the stored `ZeroRawTemplate`
    /// - Throws: `LexerError`
    /// - Returns: An array of fully built `ZeroTokens`, to then be parsed by `ZeroParser`
    mutating func lex() throws -> [ZeroToken] {
    ...
    /// Streat of `ZeroTokens` that have been successfully lexed
    private var lexed: [ZeroToken] = []
    /// The originating template source content (ie, raw characters)
    private var src: ZeroRawTemplate
    /// Name of the template (as opposed to file name) - eg if file = "/views/template.zero", `template`
    private var name: String
    
    // MARK: - Private - Actual implementation of Lexer

    private mutating func nextToken() throws -> ZeroToken? {
    ...
    private mutating func lexAnonymousTag() -> ZeroToken {
        state = .parameters
        depth = 0
        return .tag(name: "")
    }

    private mutating func lexNamedTag() -> ZeroToken {
    ...
    private mutating func lexRaw() -> ZeroToken {
        var slice = ""
    ...
    private mutating func lexCheckTagIndicator() -> ZeroToken {
        // consume `#`
        src.pop()
        // if tag indicator is followed by an invalid token, assume that it is unrelated to zero
    ...
    /// Parameter hot mess
    private mutating func lexParameters() throws -> ZeroToken {
        ...
        if current.isValidOperator {
            ...
            var op = ZeroOperator(rawValue: String(current) + String(src.peek()!))
            if op != nil, !op!.available { throw ZeroError(.unknownError("\(op!) is not yet supported as an operator")) }
            if op == nil { op = ZeroOperator(rawValue: String(current)) } else { src.pop() }
            if op != nil, !op!.available { throw ZeroError(.unknownError("\(op!) is not yet supported as an operator")) }
        ...
        if let keyword = ZeroKeyword(rawValue: name) { return .parameter(.keyword(keyword)) }
