...
// MARK: `ZeroError` Summary

/// `ZeroError` reports errors during the template rendering process, wrapping more specific
/// errors if necessary during Lexing and Parsing stages.
///
/// #TODO
/// - Implement a ParserError subtype
public struct ZeroError: Error {
    /// Possible cases of a ZeroError.Reason, with applicable stored values where useful for the type
    public enum Reason {
        ...
        // MARK: Errors related to ZeroCache access
    ...
    /// Provide  a custom description of the `ZeroError` based on type.
    ///
    ...
    /// Create a `ZeroError` - only `reason` typically used as source locations are auto-grabbed
    public init(
        _ reason: Reason,
        file: String = #file,
...
// MARK: - `LexerError` Summary (Wrapped by ZeroError)
...
public struct LexerError: Error {
    ...
    ///   - lexed: `ZeroTokens` already lexed prior to error
...
// MARK: - `ParserError` Summary (Wrapped by ZeroError)
// FIXME: Implement a specific ParserError type
